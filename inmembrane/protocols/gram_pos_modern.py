...
DeepLocPro, MASSP, pyHMMER).  Each predictor contributes features
that are merged heuristically to classify proteins as:

    - PSE-Cellwall          (anchored surface proteins)
    - PSE-Membrane          (membrane proteins with exposed loops)
    - PSE-Lipoprotein       (Sec/Tat SPII signal)
    - SECRETED              (exported, non-membrane)
    - MEMBRANE(non-PSE)     (embedded, no external loops)
    - CYTOPLASMIC(non-PSE)  (internal)
...
DEFAULT_PARAMS = {
    ...
    "massp_threshold_exposed": 0.35,        # min exposed fraction to call PSE
    "terminal_exposed_loop_min": 50,        # kept for compatibility
    "internal_exposed_loop_min": 100,
}

# Gram-positive surface-anchoring Pfam / Superfamily accessions
GRAM_POS_SURFACE_HMMS = {
    ...
    "SLH": ["PF00395"],      # S-layer homology domain
    "NLPC_P60": ["PF00877"],
}


# ---------------------------------------------------------------------
# Utility: identify if any cell-wall–anchoring motif is present
# ---------------------------------------------------------------------
...
def post_process_protein(params, protein):
    """
    Integrate predictor outputs and classify the protein.
    """
    ...
    # 1. Domain-based overrides (e.g. LPXTG, LysM, SLH)
    anchor_family = has_surface_anchor(hmm_hits)
    if anchor_family:
        details.append(f"hmm_anchor({anchor_family})")
        category = "PSE-Cellwall"

    # 2. Signal peptides dominate: if a secretion/lipoprotein signal is detected,
    #   that decision takes precedence over topology
    ...
    # 3. Membrane proteins: decide by exposure
    elif n_tms > 0:
    ...
    # 4. Localization fallback from DeepLocPro
